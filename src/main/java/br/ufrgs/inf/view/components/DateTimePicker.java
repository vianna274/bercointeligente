...
public class DateTimePicker extends DatePicker {
    ...
    public DateTimePicker() {
        ...
        getEditor().focusedProperty().addListener((observable, oldValue, newValue) -> {
            if (!newValue) {
                String underlyingVal = getEditor().getCharacters().toString();

                if (underlyingVal == null || "".equals(underlyingVal)) {
                    dateTimeValue.setValue(LocalDateTime.now());

                } else {
                    dateTimeValue.setValue(LocalDateTime.parse(underlyingVal, formatter));
                }
